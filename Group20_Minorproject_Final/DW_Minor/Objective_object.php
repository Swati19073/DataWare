...
<p style="text-align:center">Objective Object</p>
</header>
<div style="text-align:center">
<form>

<label for="alt_values1">Select Alternative </label>
<select id="alt_values1" name="Alternative">Select Alternative
...
<?php
    ...
    if($res = $conn->query($sql)){
        ...
        {
            $alternate_values1=$list["alt_name"];
            ?>

<option value="<?php echo $alternate_values1;?>"> <?php echo $alternate_values1;?></option>
...
<label for="obj_values">Select Action Objective </label>
<select id="obj_values" name="Objective">Select Action Objective
<option value="Select Action Objective"> Select Action Objective </option>

<?php
    
    
    $sql2 = "show columns from alt_objectives";
...
<input type="text" id="t_value" name="txt_val1"><br>
...
<?php
    if (isset($_GET["Alternative"]) && isset($_GET["Objective"]) && isset($_GET["txt_val1"]))
    {
        $row1=$_GET["Alternative"];
        $col1=$_GET["Objective"];
        $val1=$_GET["txt_val1"];
        
         if(!empty($row1) && !empty($col1) && !empty($val1) && $row1!="Select Alternative" && $col1!="Select Action Objective")
         {
        $sql3="update alt_objectives set $col1='$val1' where alt_name = '$row1' ";
